...
export function EditRequestedItinerary() {
  ...
  const [initialFormData, setInitialFormData] = useState<RequestedBookingFormData | null>(null);
  ...
  useEffect(() => {
    ...
    } else if (passedItineraryData.dates && typeof passedItineraryData.dates === 'string') {
      // Parse date range from string like "February 10, 2025 - February 15, 2025"
      const dateMatch = passedItineraryData.dates.match(/([\w\s,]+)\s*[-–]\s*([\w\s,]+)/);
      if (dateMatch) {
    ...
    if (passedItineraryData.totalAmount) {
      const numericValue = passedItineraryData.totalAmount.replace(/[₱$,]/g, '').trim();
      parsedAmount = numericValue;
    } else if (passedItineraryData.total) {
      const numericValue = passedItineraryData.total.replace(/[₱$,]/g, '').trim();
      parsedAmount = numericValue;
    }

    const loadedFormData: RequestedBookingFormData = {
      customerName: passedItineraryData.customer || passedItineraryData.customerName || "",
  ...
  }, [formData.travelDateFrom, formData.travelDateTo, pendingDateChange, itineraryDays]);
  ...
  const handleConfirmSave = () => {
    ...
    const updatedBooking = {
      ...
      paid: passedItineraryData?.paid || "₱0",
      bookedDate: passedItineraryData?.bookedDate || new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
      bookedDateObj: passedItineraryData?.bookedDateObj || new Date(),
      status: passedItineraryData?.status || "pending" as const,
      sentStatus: passedItineraryData?.sentStatus || "unsent" as const,
      confirmStatus: passedItineraryData?.confirmStatus || "unconfirmed" as const,
      ...
      <ConfirmationModal
        ...
        content={
          <div className="text-card-foreground">
            <p>Are you sure you want to save changes to this requested booking?</p>
          </div>
        }
