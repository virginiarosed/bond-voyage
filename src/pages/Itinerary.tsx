...
export function Itinerary({ onCreateBooking, requestedBookingsFromBookings = [], newStandardItineraries = [], drafts = [], onEditItinerary, onEditRequestedBooking, onEditRequestedDraft }: ItineraryProps) {
  ...
  // Merge with newly created itineraries and filter out deleted ones
  const templates = [...newStandardItineraries, ...defaultTemplates].filter(t => !deletedItineraries.includes(t.id));
  ...
  // Merge with itinerary details from newly created itineraries
  ...
  // Update requested itinerary details when new bookings are added
  useEffect(() => {
  ...
  const getFilteredRequestedBookings = () => {
    let filtered = requestedBookings;
  ...
  const selectedRequested = selectedRequestedId ? requestedBookings.find(b => b.id === selectedRequestedId) : null;
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    if (onEditItinerary) {
                      // Pass the full itinerary with itineraryDetails
                      const fullItinerary = {
                        ...template,
                        itineraryDetails: standardItineraryDetails[template.id] || []
                      };
                      onEditItinerary(fullItinerary);
                    }
                  }}
    ...
    <div>
      {selectedCategory === "Standard" && viewMode === "detail" && selectedStandard ? (
        ...
        <StandardItineraryDetailView
          ...
          onEdit={() => {
            if (onEditItinerary) {
              // Need to pass the full itinerary with itineraryDetails
              const fullItinerary = {
                ...selectedStandard,
                itineraryDetails: standardItineraryDetails[selectedStandard.id] || []
              };
          ...
          actionButtons={
            <div className="space-y-3">
              ...
              {selectedRequested.sentStatus === "sent" && (
                <button
                  onClick={() => handleUnsendToClient(selectedRequested.id)}
                  className="w-full h-11 px-4 rounded-xl bg-gradient-to-r from-[#FFB84D] to-[#FF9800] hover:from-[#FFA21A] hover:to-[#F57C00] text-white flex items-center justify-center gap-2 font-medium transition-all shadow-lg shadow-[#FFB84D]/20"
                >
                  <Send className="w-4 h-4" />
                  Unsend
                </button>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-[#1A2B4F] mb-1 group-hover:text-[#0A7AFF] transition-colors">New Standard Itinerary</h3>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (draft.type === "requested") {
                            ...
                            if (onEditItinerary) {
                              onEditItinerary(draft);
                            }
                          }
                        }}
                        className="w-full p-4 rounded-xl border-2 border-[#E5E7EB] hover:border-[#FFB84D] bg-white hover:bg-[rgba(255,184,77,0.02)] transition-all text-left group"
                      >
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#FFB84D] to-[#FF9800] flex items-center justify-center shadow-md">
