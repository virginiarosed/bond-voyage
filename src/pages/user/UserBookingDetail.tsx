...
import { Plane, Hotel, Camera, UtensilsCrossed, Car, Download, QrCode, Keyboard, CreditCard, User, Phone, Mail, Banknote, Smartphone, Upload, X, CheckCircle2, ChevronLeft, MapPin, Calendar as CalendarIcon, Users, Clock, Pen, Save, Copy, Shield, AlertCircle } from "lucide-react";
...
export function UserBookingDetail() {
  ...
  const handleSubmitPayment = () => {
    ...
    if (paymentType === "Partial Payment" && (!partialAmount || parseFloat(partialAmount) <= 0)) {
      toast.error("Please enter a valid amount");
      return;
    }

    if (modeOfPayment === "Gcash" && !proofOfPayment) {
      toast.error("Please upload proof of payment for Gcash");
      return;
    }

    if (modeOfPayment === "Cash" && !cashConfirmation) {
      toast.error("Please upload payment confirmation for Cash");
      return;
    }
    ...
    // Removed confetti from here - only in GCash modal Done button
  ...
  const handleModeOfPaymentClick = (mode: "Cash" | "Gcash") => {
    ...
    if (mode === "Cash") {
      // Open receipt modal for Cash payment
    ...
    <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-[0_1px_3px_rgba(0,0,0,0.08)] overflow-hidden">
            <div className="p-6 border-b border-[#E5E7EB] bg-gradient-to-br from-[#F8FAFB] to-white">
              <div className="flex items-center justify-between">
                ...
                {totalAmount - (booking.totalPaid || 0) > 0 && (
                  <button
                    onClick={() => setEditingPayment(!editingPayment)}
                    className="w-9 h-9 rounded-xl border border-[#E5E7EB] bg-white hover:border-[#0A7AFF] hover:bg-[rgba(10,122,255,0.05)] flex items-center justify-center transition-all"
                  >
                    <Pen className="w-4 h-4 text-[#64748B] hover:text-[#0A7AFF]" />
                  </button>
                )}
              </div>
            </div>
              ...
              {totalAmount - (booking.totalPaid || 0) > 0 ? (
                <>
                  ...
                  <div>
                    ...
                    <Select value={paymentType} onValueChange={handlePaymentTypeChange} disabled={!editingPayment}>
                      <SelectTrigger id="payment-type" className="h-11 border-[#E5E7EB] focus:border-[#0A7AFF] focus:ring-[#0A7AFF]/10 disabled:opacity-60 disabled:cursor-not-allowed">
                  ...
                  {paymentType === "Partial Payment" && (
                    <div>
                      <Label htmlFor="amount" className="text-[#1A2B4F] mb-2 block">Amount</Label>
                      <div className="relative">
                        ...
                        <Input
                          ...
                          placeholder="0.00"
                          className="h-11 pl-8 border-[#E5E7EB] focus:border-[#0A7AFF] focus:ring-[#0A7AFF]/10"
                          max={totalAmount - (booking.totalPaid || 0)}
                          min="0"
                          disabled={!editingPayment}
                        />
                      </div>
                      <p className="text-xs text-[#64748B] mt-1">Maximum: ₱{(totalAmount - (booking.totalPaid || 0)).toLocaleString()}</p>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          disabled={!editingPayment}
                          className={`h-12 rounded-xl border-2 flex items-center justify-center gap-2 font-medium transition-all disabled:opacity-60 disabled:cursor-not-allowed ${
                            modeOfPayment === "Cash"
                  ...
                  {modeOfPayment === "Cash" && (
                    <div>
                      <Label htmlFor="cash-confirmation-upload" className="text-[#1A2B4F] mb-2 block">
                      ...
                      {cashConfirmationPreview ? (
                        ...
                        <div className="relative">
                          ...
                          />
                          <label
                            htmlFor="cash-confirmation-upload"
                            className={`flex items-center justify-center gap-2 h-24 px-4 rounded-xl border-2 border-dashed ${
                              editingPayment 
                                ? "border-[#E5E7EB] hover:border-[#0A7AFF] hover:bg-[rgba(10,122,255,0.05)] cursor-pointer" 
                                : "border-[#E5E7EB] bg-[#F8FAFB] cursor-not-allowed opacity-60"
                            } transition-all`}
                          >
                            <div className="text-center">
                  ...
                  <div className="pt-4 border-t border-[#E5E7EB] space-y-2">
                        ...
                        {editingPayment
                          ? paymentType === "Partial Payment"
                            ...
                            : paymentType === "Full Payment"
                              ? `₱${(totalAmount - (booking.totalPaid || 0)).toLocaleString()}`
                              : `₱${(totalAmount - (booking.totalPaid || 0)).toLocaleString()}`
                          : `₱${(totalAmount - (booking.totalPaid || 0)).toLocaleString()}`
                        }
                      </span>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-[#E5E7EB]">
                      ...
                      <span className="font-semibold text-[#FF6B6B]">
                        ₱{(totalAmount - (booking.totalPaid || 0) - (editingPayment ? (paymentType === "Partial Payment" ? partialAmountNum : paymentType === "Full Payment" ? (totalAmount - (booking.totalPaid || 0)) : 0) : 0)).toLocaleString()}
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-xs text-[#64748B] mb-2">
                      ...
                      <span>{Math.round(((booking.totalPaid || 0) / totalAmount) * 100)}%</span>
                    </div>
                    <div className="h-2 bg-[#E5E7EB] rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-gradient-to-r from-[#10B981] to-[#14B8A6] transition-all duration-300"
                        style={{ width: `${((booking.totalPaid || 0) / totalAmount) * 100}%` }}
                      />
                    </div>
                  </div>
                    ...
                    <div className="pt-4 border-t border-[#E5E7EB]">
                      ...
                      <div className="space-y-3 max-h-60 overflow-y-auto">
                        {booking.paymentHistory.map((payment, index) => (
                          <div 
                            key={payment.id} 
                            className="bg-[#F8FAFB] rounded-lg p-3 border border-[#E5E7EB] cursor-pointer hover:border-[#0A7AFF] hover:bg-[rgba(10,122,255,0.02)] transition-all duration-200"
                  ...
                  {/* Submit Button */}
                  {editingPayment && paymentType && modeOfPayment && (
                    <div className="pt-4 border-t border-[#E5E7EB]">
                      <button
                ...
                <div className="text-center py-8">
                  <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-[#10B981] to-[#14B8A6] rounded-full flex items-center justify-center">
                    <CheckCircle2 className="w-8 h-8 text-white" />
                  </div>
                  <h4 className="text-lg font-semibold text-[#1A2B4F] mb-2">Fully Paid</h4>
                  <p className="text-sm text-[#64748B]">This booking has been completely paid.</p>
      ...
      <Dialog open={receiptModalOpen} onOpenChange={setReceiptModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Please present this receipt when paying at 4B'S TRAVEL AND TOURS ADMIN.
            </DialogDescription>
          </DialogHeader>
          
          {/* Receipt Content */}
          <div ref={receiptRef} className="bg-white p-6 border-2 border-dashed border-[#E5E7EB] rounded-lg">
            <div className="text-center mb-6">
              <div className="w-16 h-16 mx-auto mb-3 bg-gradient-to-br from-[#0A7AFF] to-[#14B8A6] rounded-full flex items-center justify-center">
                <span className="text-3xl">🎫</span>
              </div>
              <h3 className="text-xl font-semibold text-[#1A2B4F] mb-1">4B'S TRAVEL AND TOURS</h3>
              <p className="text-sm text-[#64748B]">Official Payment Receipt</p>
            </div>

            <div className="space-y-3 mb-6">
              <div className="flex justify-between pb-2 border-b border-[#E5E7EB]">
                <span className="text-sm text-[#64748B]">Receipt ID</span>
                <span className="text-sm font-semibold text-[#1A2B4F]">{booking.id}</span>
              </div>
              <div className="flex justify-between pb-2 border-b border-[#E5E7EB]">
                <span className="text-sm text-[#64748B]">Customer</span>
                <span className="text-sm font-semibold text-[#1A2B4F]">{booking.customer}</span>
              </div>
              <div className="flex justify-between pb-2 border-b border-[#E5E7EB]">
                <span className="text-sm text-[#64748B]">Destination</span>
                <span className="text-sm font-semibold text-[#1A2B4F]">{booking.destination}</span>
              </div>
              <div className="flex justify-between pb-2 border-b border-[#E5E7EB]">
                <span className="text-sm text-[#64748B]">Payment Type</span>
                <span className="text-sm font-semibold text-[#1A2B4F]">{paymentType}</span>
              </div>
              <div className="flex justify-between pb-2 border-b border-[#E5E7EB]">
            ...
            <div className="bg-[rgba(10,122,255,0.05)] rounded-lg p-4 text-center">
              <p className="text-xs text-[#64748B] mb-1">Mode of Payment</p>
              <p className="text-sm font-semibold text-[#0A7AFF]">CASH</p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full h-11 px-4 rounded-xl bg-gradient-to-r from-[#0A7AFF] to-[#14B8A6] text-white font-medium hover:shadow-lg transition-all flex items-center justify-center gap-2"
          >
            <Download className="w-4 h-4" />
            Download Receipt
          </button>
      ...
      <Dialog open={gcashModalOpen} onOpenChange={setGcashModalOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden border-0 shadow-2xl max-h-[90vh] flex flex-col">
          ...
          {/* Main Content Area */}
          <div className="bg-[#0A7AFF] flex-1 overflow-y-auto p-8">
            ...
            <div className="bg-white rounded-2xl">
              <div className="p-4 space-y-4">
                ...
                {/* QR Code Section - Only show when QR tab is active */}
                {paymentMethod === 'qr' && (
                  <div className="text-center">
                ...
                <div className="bg-white rounded-xl p-4 border border-[#E5E7EB] space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-[#E5E7EB]">
                    ...
                    <span className="text-sm font-semibold text-[#1A2B4F]">4B'S TRAVEL AND TOUR</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-[#E5E7EB]">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="w-8 h-8 rounded-lg order border-[#E5E7EB] hover:bg-[#0A7AFF] hover:border-[#0A7AFF] hover:text-white transition-all flex items-center justify-center group"
                      >
                        <Copy className="w-3.5 h-3.5 text-[#64748B] group-hover:text-[#0A7AFF]" />
                      </button>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                ...
                {/* Secure Note */}
                <div className="bg-[#F8FAFB] rounded-xl p-3 border-l-4 border-l-[#0A7AFF]">
                  <div className="flex items-start gap-2">
                    <Shield className="w-4 h-4 text-[#0A7AFF] mt-0.5 shrink-0" />
                    <div>
                      <p className="text-sm font-medium text-[#1A2B4F] mb-1">Secure Transaction</p>
                      <div className="text-xs text-[#64748B] space-y-0.5">
                        <p>• Verify <strong>4B'S TRAVEL AND TOUR</strong> as recipient</p>
          ...
          <div className="p-6 border-t border-[#E5E7EB] bg-white shrink-0">
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 h-11 px-4 rounded-xl bg-[#0A7AFF] text-white font-medium hover:bg-[#0057B8] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Done
              </button>
            </div>
