import { Calendar, MapPin, User, Users, Globe, Eye, Edit, RotateCcw, X, ChevronRight, Phone, Mail, CreditCard, CheckCircle2, Package, Clock, Plane, Hotel, Camera, UtensilsCrossed, Car, Briefcase, FileCheck, ClipboardList, BookOpen } from "lucide-react";
...
interface BookingsProps {
  ...
  onMoveToHistory: (booking: Booking, status: "completed" | "cancelled") => void;
...
export function Bookings({ onMoveToApprovals, onMoveToRequested, onMoveToHistory, createdBookings = [], onBookingsCountChange }: BookingsProps) {
  ...
  }, [viewMode, selectedBookingId, setBreadcrumbs, resetBreadcrumbs]);
  ...
  // Merge created bookings from Itinerary page
  useEffect(() => {
    if (createdBookings.length > 0) {
      setBookings(prevBookings => {
        const existingIds = new Set(prevBookings.map(b => b.id));
        const newBookings = createdBookings.filter(b => !existingIds.has(b.id));
  ...
  useEffect(() => {
    if (viewMode === "detail" && selectedBookingId) {
      ...
      if (currentBooking) {
        setBreadcrumbs(["Home", "Bookings", `Booking ${currentBooking.id}`]);
      }
    } else {
      resetBreadcrumbs();
    }
  ...
  const getActiveBookingsLabel = () => {
    switch (selectedStatus) {
      case "paid":
        return "Active Paid Bookings";
      case "partial":
        return "Active Partial Bookings";
      case "unpaid":
        return "Active Unpaid Bookings";
      default:
  ...
  const handleConfirmCancel = () => {
    if (bookingToCancel) {
      ...
      onMoveToHistory(bookingToCancel, "cancelled");
          ...
          content={
            bookingToCancel && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-[rgba(255,107,107,0.2)]">
                  <div>
                    <p className="text-xs text-[#64748B] mb-1">Booking ID</p>
