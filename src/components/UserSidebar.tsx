import { Home, Plane, Sparkles, Calendar, Archive, MessageSquare, Star, Grid3x3, Bell, X, ChevronRight, User, Sun, Moon, Check, ArrowLeftRight, CloudRain, Languages, Disc, LogOut, Car } from "lucide-react";
...
export function UserSidebar({ currentTheme, onThemeChange, isMobileMenuOpen, setIsMobileMenuOpen }: UserSidebarProps) {
  ...
  useEffect(() => {
    ...
    if (showUserMenu || showNotificationMenu || showThemeMenu || showSideMenu || showMoreMenu) {
      document.addEventListener('mousedown', handleClickOutside);
      return () => document.removeEventListener('mousedown', handleClickOutside);
    }
  }, [showUserMenu, showNotificationMenu, showThemeMenu, showSideMenu, showMoreMenu]);
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-screen w-20 bg-sidebar border-r border-sidebar-border flex flex-col items-center shadow-[0_0_20px_rgba(0,0,0,0.03)] z-50 transition-transform duration-300 ${
          ...
          {showSideMenu && (
            <div className="absolute left-20 top-0 ml-3 w-56 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-50 animate-in fade-in slide-in-from-left-2 duration-200">
              ...
              >
                ...
                <span className="text-sm text-card-foreground">Switch to Admin Side</span>
              </button>
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col items-center gap-2">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`w-10 h-10 rounded-[10px] flex items-center justify-center ${isActive ? '' : 'group-hover:bg-sidebar-accent'}`}>
                <Icon 
                  className={`w-5 h-5 ${isActive ? 'text-sidebar-primary' : 'text-sidebar-foreground/60'} transition-colors`} 
                  strokeWidth={2.5} 
                />
              </div>
              <span className={`text-[10.5px] ${isActive ? 'text-sidebar-primary font-semibold' : 'text-sidebar-foreground/60 font-medium'} tracking-tight transition-all`}>
                {item.label}
              </span>
            </button>
          );
        ...
        <div className="relative" ref={moreMenuRef}>
          ...
          >
            ...
            <span className="text-[10.5px] text-sidebar-foreground/60 font-medium tracking-tight transition-all">More</span>
          </button>
              ...
              {moreMenuItems.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => handleNavigate(item.id)}
                    className={`w-full px-4 py-3 flex items-center gap-3 hover:bg-accent transition-colors ${
                      index < moreMenuItems.length - 1 ? 'border-b border-border' : ''
                    }`}
                  >
      ...
      <div className="mt-auto mb-3 flex flex-col items-center gap-2">
        ...
        <div className="relative" ref={notificationMenuRef}>
          ...
          >
            <Bell className="w-5 h-5 text-sidebar-foreground/60 group-hover:animate-[wiggle_0.5s_ease-in-out]" strokeWidth={2.5} />
            {unreadCount > 0 && (
            ...
            <div className="fixed bottom-3 left-24 w-96 max-w-[calc(100vw-6rem)] bg-card/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(10,122,255,0.12),0_2px_8px_rgba(0,0,0,0.08)] border border-border/50 z-[200] overflow-hidden">
              <div className="p-5 border-b border-border/50 bg-gradient-to-br from-primary/5 to-accent/5">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-base text-popover-foreground font-semibold">Notifications</h3>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground/60 mt-1">{notification.timestamp}</p>
                      </div>
                    </div>
                  </button>
                ))}
            ...
            <div className="fixed bottom-3 left-24 w-72 max-w-[calc(100vw-6rem)] bg-card/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(10,122,255,0.12),0_2px_8px_rgba(0,0,0,0.08)] border border-border/50 z-[200] overflow-hidden">
              <div className="p-5 border-b border-border/50 bg-gradient-to-br from-primary/5 to-accent/5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-base text-popover-foreground">{userProfileData.firstName} {userProfileData.lastName}</h3>
              ...
              <div className="p-2">
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-popover-foreground">Edit Profile</p>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground capitalize">{currentTheme} mode</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-popover-foreground transition-colors" />
                </button>
      ...
      {showThemeMenu && (
        <div ref={themeMenuRef} className="fixed bottom-3 left-24 lg:left-[400px] w-80 max-w-[calc(100vw-6rem)] bg-card/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(10,122,255,0.12),0_2px_8px_rgba(0,0,0,0.08)] border border-border/50 z-[200] overflow-hidden">
          <div className="p-5 border-b border-border/50 bg-gradient-to-br from-warning/5 to-warning/10">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-base text-popover-foreground">Theme Settings</h3>
          ...
          <div className="p-3">
            ...
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-colors ${
                ...
                <Sun className={`w-5 h-5 ${currentTheme === "light" ? 'text-primary' : 'text-muted-foreground'}`} strokeWidth={2.5} />
              </div>
              <div className="flex-1 text-left">
                <p className={`text-sm font-medium ${currentTheme === "light" ? 'text-primary' : 'text-popover-foreground'}`}>Light</p>
            ...
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-colors ${
                ...
                <Moon className={`w-5 h-5 ${currentTheme === "dark" ? 'text-primary' : 'text-muted-foreground'}`} strokeWidth={2.5} />
              </div>
              <div className="flex-1 text-left">
                <p className={`text-sm font-medium ${currentTheme === "dark" ? 'text-primary' : 'text-popover-foreground'}`}>Dark</p>
