...
export function TopNav({ pageTitle, pageSubtitle, breadcrumbs, isMobileMenuOpen, onMobileMenuToggle }: TopNavProps) {
  ...
  const handleBreadcrumbClick = (item: BreadcrumbItem, index: number) => {
    // Only navigate if it's not the last item and has a path
    if (index < breadcrumbs.length - 1 && item.path) {
      navigate(item.path);
    }
  };

  return (
    <div className="sticky top-0 bg-card border-b border-border shadow-[0_1px_3px_rgba(0,0,0,0.08)] z-30 px-4 sm:px-6 lg:px-8 py-4 sm:py-5 lg:py-6">
