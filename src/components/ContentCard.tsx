...
export function ContentCard({ title, icon: Icon, action, children, footer }: ContentCardProps) {
  return (
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          {title && (
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg text-card-foreground font-semibold">{title}</h2>
            </div>
          )}
          {action && (
